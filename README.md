...
2. `--min-score`、低品質ページ除外、作家名/作品名の妥当性で品質フィルタ
3. 最終出力では同一ソースを `--max-per-source-final` 件までに制限
4. 最終出力では同一記事タイプを `--max-per-article-type-final` 件までに制限
5. 条件が厳しすぎて `--limit` 件に届かない場合は、無理に10件へ水増しせず、取得できた良記事だけを出力する
...
NotebookLMやChatGPTに入れる通常ファイルは、最終選抜後の10件程度だけを出力します。`output.md` の冒頭には以下の選抜条件も出します。
...
```powershell
py -m street_photo_collector --candidate-limit 60 --limit 10 --per-source 8 --min-score 12 --max-per-source-final 2 --max-per-article-type-final 4
```

`python` コマンドが使える環境では以下でも動きます。

```bash
python -m street_photo_collector --candidate-limit 60 --limit 10 --per-source 8 --min-score 12 --max-per-source-final 2 --max-per-article-type-final 4
```

取得済み URL も含めて再出力を確認したい場合:

```bash
python -m street_photo_collector --candidate-limit 60 --limit 10 --per-source 8 --min-score 12 --max-per-source-final 2 --max-per-article-type-final 4 --include-seen
```
